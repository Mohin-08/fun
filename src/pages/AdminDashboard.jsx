...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubOrgs = onValue(orgsRef, (s) =>
      setOrgs(
        s.exists()
    ...
    const unsubUsers = onValue(usersRef, (s) => {
      console.log("Users data:", s.exists() ? s.val() : "No users");
      setUsers(
        s.exists()
          ? Object.entries(s.val()).map(([id, v]) => ({
              id,
    ...
    const unsubComplaints = onValue(complaintsRef, (s) =>
      setComplaints(
        s.exists()
          ? Object.entries(s.val()).map(([id, v]) => ({
              id,
    ...
    const unsubAuditLogs = onValue(auditLogsRef, (s) => {
  ...
  const createOrganization = async () => {
    if (!orgName || !emailDomain) return alert("Fill all fields");
    ...
    logAdminAction(`Created organization ${orgName}`);
    setOrgName("");
    setEmailDomain("");
  };
  ...
  const getManagerStats = () => {
    ...
    return managers.map(manager => {
      ...
      return {
        id: manager.id,
        name: manager.name || manager.email?.split('@')[0] || 'Unknown',
        email: manager.email,
        orgId: manager.orgId,
